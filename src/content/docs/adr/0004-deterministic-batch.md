...
- **狀態推進批次的冪等性。** 重複呼叫 `/v1/batch/run` 會把案件再往前推一步。
  交換匯入已經是冪等的([ADR-0005](/adr/0005-batch-idempotency/))，但狀態推進
  不能用同樣的方法——原因見那一篇的最後一節。
...
:::tip[決定性與冪等是兩件事]
**決定性**是「同一批輸入產生同一批輸出」，講的是單次執行內部沒有隨機性。
**冪等**是「執行一次和執行 N 次結果相同」，講的是重複執行之間的關係。

這個 API 的兩種批次工作都是決定性的，但只有交換匯入是冪等的。
:::

...
批次要把案件往下一個狀態推。示範系統很容易走上這條路:用亂數決定核准或婉拒,
讓輸出看起來「有變化」、比較像真的。

但批次作業真正困難的地方從來不是產生變化,是**解釋變化**。實務上最花時間的問題
長這樣:昨天的報表數字對不上,要回答「這批到底做了什麼」。如果批次本身帶有隨機性,
這個問題無解——你不能重跑一次來比對,因為重跑會得到不同結果。

我維護過的系統裡,批次是由資料庫排程直接搬資料。那種批次的除錯困難不在於它會出錯,
而在於**它跑完之後你不知道它做了什麼**——只能反過來從資料的變化去推論。
所以這裡的設計把重點放在「回報做了什麼」,而不是「回報成功」。

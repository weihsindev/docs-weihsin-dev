...
把規則集中之後,「還有哪裡要改」變成一個看得完的清單,而不是一件要靠記憶的事。

:::note[這一篇是設計選擇,不是經驗教訓]
和 [ADR-0004](/adr/0004-validation-in-app-not-sp/)、[ADR-0005](/adr/0005-per-row-failure-reporting/)
不同,這篇沒有對應到我實際踩過的坑——我維護過的系統沒有出現過狀態規則散落各處的問題。
它是這個專案裡的設計選擇,理由如上,代價如下。
:::

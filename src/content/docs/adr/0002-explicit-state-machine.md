...
我在既有系統看過這個做法的終點。規則寫在 Stored Procedure 裡,想複用就複製一支新的
出來改([ADR-0004](/adr/0004-validation-in-app-not-sp/))。久了會有好幾支長得很像但
不完全一樣的 SP,而且**它們會分岔**——本來一樣的兩支,其中一支改了,另一支沒改,
行為從此不一致,沒有任何機制會提醒你。要維護時得先花時間確認它們到底哪裡不一樣。

分岔的根源不是複製本身,是**沒有一個「唯一的真相」可以指向**。

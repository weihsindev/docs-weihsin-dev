...
**但它不是原系統的複製品。** 我維護的進件流程是資料庫導向的:檢核寫在 Stored Procedure,
通過之後往另一張表插一筆,那一筆就是進件成立的依據。這個 API 是把同一個流程改用
顯式契約重新表達,並且針對原做法的三個問題做出不同的選擇——追溯性、規則分岔、
以及使用者感受得到的同步延遲。

那三個問題與取捨寫在 [ADR-0004](/adr/0004-validation-in-app-not-sp/),
那篇也說明了原本的做法在什麼條件下仍然是對的。
